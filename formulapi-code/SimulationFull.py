...
setDriveUrl = r'http://%s:%d/?m1=%%.2f&m2=%%.2f&l1=%%d&reset=%%d' % (simulationIP, simulationPort)
...
def YetiMotors(driveLeft, driveRight):
	...
	print RaceCodeFunctions.GetDistance()
	SendToSimulation()
...
def SendToSimulation():
	...
	global simRight
	global toReset;
	url = setDriveUrl % (simLeft, simRight, simLed, Globals.toReset)
	if Globals.toReset==1:
		Globals.toReset = 0
	try:
